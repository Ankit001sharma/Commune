...
const ListingCard = ({ listing }) => {
  ...
  const handleFavorite = async (e) => {
    ...
    try { await toggleFavorite(listing._id); } catch (_) {}
  };
  ...
  return (
    <div className="card" onClick={() => navigate(`/marketplace/${listing._id}`)} style={{ cursor: 'pointer' }}>
      {imageUrl ? (
        <img src={imageUrl} alt={listing.title} className="card-image" loading="lazy" />
      ) : (
      ...
      <div className="card-body">
        ...
        <div className="card-meta">
          ...
          {listing.negotiable && <span className="badge badge-success">Negotiable</span>}
        </div>
      </div>
      <div className="card-footer">
        <div className="card-seller">
          <div className="card-seller-avatar">{getInitials(listing.seller)}</div>
          <span>{listing.seller?.firstName} {listing.seller?.lastName}</span>
        </div>
        <div className="card-actions">
          <button className={`card-action-btn ${isFavorited ? 'favorited' : ''}`} onClick={handleFavorite} title="Favorite">
